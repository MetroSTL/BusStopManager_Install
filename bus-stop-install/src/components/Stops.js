    ...
    render() {
        ...
        return (
            <div>
                {features.map(feat=>{
                    return (
                    <button key={feat.attributes.objectid} onClick={(e)=>this.onClick(e, feat.attributes.globalid)}>
                ...
                <div  id='icontainer'>
                    ...
                    <button onClick={()=>document.getElementById('icontainer').style.display = 'none'}>Close</button>
                </div>
            </div>
        )
    }
